...
export function useGridPositions(board: BoardContext) {
  ...
  const fullWidth = computed(() => gameGridWidth.value + leftGutterWidth.value + rightGutterWidth.value);
  const fullHeight = computed(() => gameGridHeight.value + topGutterHeight.value + bottomGutterHeight.value);
