    ...
    <BoardCells>
      ...
      <template v-slot:cell="{ row, col }">
        <div
          ...
          :class="[{
            'bg-[url(/assets/minesweeper/unopened-square.svg)]': [MinesweeperCell.SAFE, MinesweeperCell.FLAG, MinesweeperCell.QUESTION_MARK].includes(state.board[row][col])},
          ...
          <img v-if="state.board[row][col] === MinesweeperCell.UNMARKED_HIGHLIGHTED" src="/assets/minesweeper/unopened-square-highlighted.svg" alt="Unmarked" class="w-full h-full" />
          <img v-if="state.board[row][col] === MinesweeperCell.FLAG" src="/assets/minesweeper/flag.svg" alt="Flagged" class="w-full h-full" />
          <img v-if="state.board[row][col] === MinesweeperCell.QUESTION_MARK" src="/assets/minesweeper/question-mark.svg" alt="Question Mark" class="w-full h-full" />
