...
/**
 * MinesweeperFreeplay - Freeplay mode wrapper for Minesweeper
 *
...
function on_cell_click(row: number, col: number, button: number) {
  ...
  if (result) {
    // Store the target value for subsequent drag operations
...
async function request_new_puzzle() {
  ...
  game.value = useMinesweeperGame(new_def, null);
