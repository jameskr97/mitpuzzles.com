...
// Fall back to default puzzle if none available from API
const initial_definition: PuzzleDefinition<AquariumMeta> = services.definition.value ?? {
  ...defaultPuzzles.aquarium.definition,
  id: 0,
  puzzle_type,
} as PuzzleDefinition<AquariumMeta>;

const game = shallowRef<AquariumGameReturn>(
  useAquariumGame(initial_definition, services.definition.value ? services.saved_board.value : null)
);
...
async function request_new_puzzle() {
  ...
  if (new_def) {
    game.value = useAquariumGame(new_def, null);
    ui.value.show_solved_state = false;
