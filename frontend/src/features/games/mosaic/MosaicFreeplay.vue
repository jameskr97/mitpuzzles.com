...
/**
 * MosaicFreeplay - Freeplay mode wrapper for Mosaic
 */
import { computed, ref, watch, shallowRef } from "vue";
import { useMosaicGame, type MosaicGameReturn } from "./useMosaicGame";
...
async function request_new_puzzle() {
  ...
  game.value = useMosaicGame(new_def, null);
  ui.value.show_solved_state = false;
...
<template>
  <GameLayout :controller="controller" :definition="controller.state.value.definition">
    <MosaicCanvas
      :key="canvas_key"
      :state="puzzle_state"
      :get_number_clue="game.get_number_clue"
