...
export function useSudokuFocusHighlighter(ctrl: PuzzleController) {
  ...
  const setShouldRecordFocus = (value: HistoryMode | null) => {
     shouldRecordFocus.value = value;
     if (value === null) return;
     recorder.set_mode(value);
  }
  
  // Use game recorder for consistent recording across freeplay and experiment modes
  ...
  const inputBehavior: Partial<BoardEvents> = {
    ...
    onCellFocus(cell: Cell, event: MouseEvent): boolean {
      if (!enabled.value) return true;
      lastSelectedHoveredCell.value = cell;
      ctrl.handle_cell_focus?.(cell, event)
      return false;
    },
  };
