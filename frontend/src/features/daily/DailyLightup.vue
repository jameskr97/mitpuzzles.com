...
import { useLightupGame, type LightupGameReturn } from "@/features/games/lightup/useLightupGame";
...
const puzzle_state = computed(() => ({ definition: game.value.definition, board: game.value.board.value, solved: services.is_solved.value, immutable_cells: game.value.immutable_cells.value, violations: violations.value, lit_cells: game.value.lit_cells.value }));
...
<template>
  <DailyGameLayout :controller="controller" :definition="controller.state.value.definition" :date="services.date" :error="services.error.value">
    <LightupCanvas :key="canvas_key" :state="puzzle_state" @cell-click="on_cell_click" @cell-drag="on_cell_drag" @cell-enter="on_cell_enter" @cell-leave="on_cell_leave" />
  </DailyGameLayout>
</template>
