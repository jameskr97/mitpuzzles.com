...
import { useMinesweeperGame, type MinesweeperGameReturn } from "@/features/games/minesweeper/useMinesweeperGame";
...
const puzzle_type = "minesweeper";
const services = await useDailyServices(puzzle_type);
const progress_key = `daily:${puzzle_type}`;

const game = shallowRef<MinesweeperGameReturn>(
  useMinesweeperGame(services.definition.value!, services.saved_board.value)
);
...
<template>
  <DailyGameLayout :controller="controller" :definition="controller.state.value.definition" :date="services.date" :error="services.error.value">
    <MinesweeperCanvas :key="canvas_key" :state="puzzle_state" @cell-click="on_cell_click" @cell-drag="on_cell_drag" @cell-enter="on_cell_enter" @cell-leave="on_cell_leave" />
  </DailyGameLayout>
</template>
