...
import { useKakurasuGame, type KakurasuGameReturn, KakurasuCell } from "@/features/games/kakurasu/useKakurasuGame";
...
const services = await useDailyServices<KakurasuMeta>(puzzle_type);
const progress_key = `daily:${puzzle_type}`;

const game = shallowRef<KakurasuGameReturn>(
  useKakurasuGame(services.definition.value!, services.saved_board.value)
);
...
<template>
  <DailyGameLayout :controller="controller" :definition="controller.state.value.definition" :date="services.date" :error="services.error.value">
    <KakurasuCanvas :key="canvas_key" :state="puzzle_state" @cell-click="on_cell_click" @cell-drag="on_cell_drag" @cell-enter="on_cell_enter" @cell-leave="on_cell_leave" @gutter-click="on_gutter_click" />
  </DailyGameLayout>
</template>
