...
import { useSudokuGame, type SudokuGameReturn } from "@/features/games/sudoku/useSudokuGame";
...
function on_cell_click(row: number, col: number) { last_selected_cell.value = { row, col }; }
...
<template>
  <DailyGameLayout :controller="controller" :definition="controller.state.value.definition" :date="services.date" :error="services.error.value">
    <SudokuCanvas :key="canvas_key" :state="puzzle_state" :is_prefilled="game.is_prefilled" :box_size="game.box_size.value" @cell-click="on_cell_click" @cell-key="on_cell_key" @cell-enter="on_cell_enter" @cell-leave="on_cell_leave" />
    <template #controls>
