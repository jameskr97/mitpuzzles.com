...
import { useNonogramsGame, type NonogramsGameReturn } from "@/features/games/nonograms/useNonogramsGame";
...
import type { NonogramMeta } from "@/core/games/types/puzzle-types";

const puzzle_type = "nonograms";
const services = await useDailyServices<NonogramMeta>(puzzle_type);
const progress_key = `daily:${puzzle_type}`;

const game = shallowRef<NonogramsGameReturn>(
  useNonogramsGame(services.definition.value!, services.saved_board.value)
);
...
<template>
  <DailyGameLayout :controller="controller" :definition="controller.state.value.definition" :date="services.date" :error="services.error.value">
    <NonogramsCanvas :key="canvas_key" :state="puzzle_state" @cell-click="on_cell_click" @cell-drag="on_cell_drag" @cell-enter="on_cell_enter" @cell-leave="on_cell_leave" />
  </DailyGameLayout>
</template>
