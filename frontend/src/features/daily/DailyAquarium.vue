...
import type { AquariumMeta } from "@/features/games/aquarium/useAquariumGame";

const puzzle_type = "aquarium";
const services = await useDailyServices<AquariumMeta>(puzzle_type);
const progress_key = `daily:${puzzle_type}`;

const game = shallowRef<AquariumGameReturn>(
  useAquariumGame(services.definition.value!, services.saved_board.value)
);
...
const puzzle_state = computed(() => ({ definition: game.value.definition, board: game.value.board.value, solved: services.is_solved.value, violations: violations.value }));
