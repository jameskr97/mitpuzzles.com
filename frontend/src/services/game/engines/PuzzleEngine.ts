...
import { RunLengthEncoder } from "@/services/game/engines/solutions.ts";
...
export class PuzzleEngine<TMeta = any> {
  ...
  public readonly initial_state: number[][];
  ...
  constructor(
    ...
    // initialize solution checker with solution hash or expected solution
    const expected_solution = definition.solution
      ? PuzzleConverter.fromResearch(definition.solution as ResearchGrid, definition.puzzle_type)
      : undefined;

    this.solution_checker = new SolutionValidator(
      definition.puzzle_type,
      definition.solution_hash,
      expected_solution
    );
