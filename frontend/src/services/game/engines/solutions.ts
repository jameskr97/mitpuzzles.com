...
export class RunLengthEncoder {
  private static readonly PUZZLE_CONFIGS: Record<string, PuzzleConfig> = {
    ...
    nonograms: {
      positiveValues: [NonogramsCell.BLACK], // Black cells (marked)
      negativeValues: [NonogramsCell.CROSS], // Cross marks (pertinent negative)
      immutableValues: [], // No immutable cells
      emptyValues: [NonogramsCell.EMPTY], // Empty cells
    },
    battleships: {
      positiveValues: [BattleshipsCell.SHIP], // Ship parts
...
export class SolutionValidator {
  ...
  private solution_hash: string | null = null;
  private expected_solution: DeveloperGrid<CellType> | null = null;

  constructor(
    private puzzle_type: string,
    solution_hash?: string,
    expected_solution?: DeveloperGrid<CellType>,
  ) {
    this.encoder = new RunLengthEncoder(puzzle_type);
    this.solution_hash = solution_hash || null;
    this.expected_solution = expected_solution || null;
  }
  ...
  async validateSolution(currentBoard: DeveloperGrid<CellType>): Promise<boolean> {
    ...
    let target_hash = this.solution_hash;

    // generate hash from expected solution if not provided
    if (!target_hash && this.expected_solution) {
      target_hash = await this.encoder.createRunLengthEncoding(this.expected_solution);
    }

    return currentHash === target_hash;
  }
}
